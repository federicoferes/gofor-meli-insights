...
const Dashboard = () => {
  ...
  const displayPerformanceData = fallbackPerformanceData; // Replace when real data is available
  const displayCostData = costData?.length > 0 ? costData : fallbackCostDistributionData;
  const displayTopProducts = topProducts?.length > 0 ? topProducts : fallbackTopProducts;
  const displayProvinceData = provinceData?.length > 0 ? provinceData : [];
  
  // Calculate metrics from salesSummary
  const gmv = salesSummary?.gmv || 1234567;
  const orders = salesSummary?.orders || 4856;
  const avgTicket = salesSummary?.avgTicket || 254.23;
  const conversion = salesSummary?.conversion || 4.2;
  const visits = salesSummary?.visits || 115683;
  const ctr = 3.8; // Replace with real data when available

  // Calculate percent changes
  const gmvChange = prevSalesSummary?.gmv 
    ? ((salesSummary?.gmv - prevSalesSummary?.gmv) / prevSalesSummary?.gmv * 100).toFixed(1)
    : '+15.3';
  
  const ordersChange = prevSalesSummary?.orders 
    ? ((salesSummary?.orders - prevSalesSummary?.orders) / prevSalesSummary?.orders * 100).toFixed(1)
    : '+8.7';
  
  const ticketChange = prevSalesSummary?.avgTicket 
